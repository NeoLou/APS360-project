...
# Bypass SSL certificate verification (don't worry about it)
# import certifi
...
def plot_popularity_histogram(df, col_name='popularity', bins=10):
     # Plot histogram
    ...
    plt.savefig('popularity_rank_distribution.png') # Save
    plt.show() # Show
    plt.close() # Close

# Calling the API
def collect_data(latest_year=2023, earliest_year=2000, num_animes=40, num_seasons=4, sort="anime_num_list_users"):
    # API query settings
    limit = "&limit={}".format(num_animes) # Set limit for num animes to get per season
    sort_param = "?sort={}".format(sort) # Sort before getting data
    ...
    # Loop through years, from latest to earliest
    for year in range(latest_year, earliest_year, -1):
        ...
        for i in range(num_seasons):
            ...
            try:
                ...
                resp = requests.get(base_url + str(year) + "/" + season + sort_param + fields + limit + nsfw, headers=headers)
            ...
            #time.sleep(0.25) # Rate limit for api
    return df_json

############################ MAIN FUNCTION ############################
if __name__ == '__main__':
    # Call collect_data() to get df of anime data
    anime_df = collect_data()

    # Show popularity distribution of df and save plot
    plot_popularity_histogram(anime_df)

    # Once all data is in df, convert it to csv, excel, json in current folder
    #anime_df.to_csv('anime_data.csv', index=False) # Convert df to csv
    #anime_df.to_excel('anime_data.xlsx', index=False) # Convert df to excel
    #save_df_to_json(anime_df, 'anime_data.json') # Convert df to json
